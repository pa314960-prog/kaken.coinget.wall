...
2. **画面から自分が外れて**（一歩下がる、フレームアウトするなど）から、
   **② 背景を記憶**を押してください。これで「自分がいない状態」の映像を
   ...
   - うまく反応しない場合は、下の「感度」スライダーを調整し、
     「シルエットマスクを表示」にチェックを入れて、自分の輪郭に
     緑色のマスクが追従するか確認しながら調整してください。
3. 画面内に戻り、**③ ゲーム開始**を押すと60秒のゲームが始まります。
   落ちてくる🪙(コイン)に自分のシルエットで触れるとスコア加算、
   👾(敵)に触れるとスコアが減りパワーダウン（少しの間、当たり判定が
   小さくなる）します。コインを10枚集めるごとに BIG 状態（8秒間、
   得点2倍・コインが金色に光る）になります。
